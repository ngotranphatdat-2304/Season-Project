...
// Adjust paths based on your actual directory structure relative to this script
const inputPath = path.resolve(
  __dirname,
  "../../season_data/All-Products.json",
);
const bigsizePath = path.resolve(__dirname, "../../season_data/big-size.json");
const outputPath = path.resolve(
  __dirname,
  "../../season_data/All-Products-Normalized.json",
);
const mediumSizePath = path.resolve(
  __dirname,
  "../../season_data/medium-size.json",
);
...
const normalizeNewData = () => {
  ...
  data.products.forEach((product: any) => {
    ...
    let frameType = "";
    if (bigSizeProduct.includes(baseName)) {
      frameType = "Big Size";
    } else if (mediumSizeProduct.includes(baseName)) {
      frameType = "Medium Size";
    } else if (smallSizeProduct.includes(baseName)) {
      frameType = "Small Size";
    }
