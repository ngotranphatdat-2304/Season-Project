...
public class ProductSearchService {
    ...
    // Nâng cấp signature nhận đủ 6 tham số để đồng bộ logic lọc sau tìm kiếm của Node.js
    public ProductSearchResponseData searchProducts(
            String q,
            String type,
            String gender,
            Boolean sale,
        ...
        List<SearchProductResponse> allMatches = productRepository.findAll().stream()
                ...
                // Lọc theo ProductType ("Eyeglasses" hoặc "Sunglasses")
                .filter(product -> type == null || type.trim().isEmpty() 
                        || type.equalsIgnoreCase(product.getType().name()) 
                        || type.equalsIgnoreCase(product.getType().getValue()))
                // Lọc theo Gender ("Male", "Female", "Unisex")
                .filter(product -> gender == null || gender.trim().isEmpty() 
                        || gender.equalsIgnoreCase(product.getSpecifications().getGender().name()))
                // Lọc theo chương trình giảm giá (salePercent > 0)
                .filter(product -> !Boolean.TRUE.equals(sale) || product.getSalePercent() > 0)
